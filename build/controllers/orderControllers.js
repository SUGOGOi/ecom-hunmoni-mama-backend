...
import { invalidateCache, reduceStock, updateStock, } from "../utils/features.js";
...
export const myOrders = async (req, res, next) => {
    try {
        const { _id } = req.query;
        if (!_id)
            return next(new ErrorHandler("Invaid user ID", 400));
        let orders = [];
...
export const specificOrders = async (req, res, next) => {
    try {
