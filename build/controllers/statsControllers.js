...
import { calculatePercentage, getChartData, getChartDataProduct, } from "../utils/features.js";
export const getDashboradStats = async (req, res, next) => {
    try {
        let stats;
        let count;
        if (myCahe.has("admin-stats")) {
            ...
            const [thisMonthProducts, thisMonthOrders, thisMonthUsers, lastMonthProducts, lastMonthOrders, lastMonthUsers, productsWithCategory, userCount, allOrders, lastSixMonthOrders, categories, lastFiveOrders, maleUser, productOutOfStock,] = await Promise.all([
...
export const getDashboradBar = async (req, res, next) => {
    try {
        let barCharts;
        const today = new Date();
        const sixMonthAgo = new Date();
...
export const getDashboradPie = async (req, res, next) => {
    try {
        let pieCharts;
        if (myCahe.has("admin-pie-chart")) {
            ...
            const [processingOrder, shippedOrder, deliveredOrder, productsWithCategory, categories, allOrders, adminCount, userCount,] = await Promise.all([
            ...
            const marketingCost = Math.round((Number(process.env.MARKET) / 100) * grossIncome);
...
export const getDashboradLine = async (req, res, next) => {
    try {
        let lineCharts;
        const today = new Date();
        const twelveMonthAgo = new Date();
